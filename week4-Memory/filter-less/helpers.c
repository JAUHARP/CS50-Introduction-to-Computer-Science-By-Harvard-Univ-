...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            image[i][j].rgbtRed = sepiaRed;     // red
            image[i][j].rgbtGreen = sepiaGreen; // green
            image[i][j].rgbtBlue = sepiaBlue;   // blue
        }
    }
    return;
}
...
// Blur image
void blur(int height, int width, RGBTRIPLE image[height][width]) // void
{
    RGBTRIPLE temp[height][width];
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            float tempRed = 0, tempGreen = 0, tempBlue = 0, pixelCount = 0; // float value
            ...
            for (int local_i = i - 1; local_i <= i + 1; local_i++)
            {
                for (int local_j = j - 1; local_j <= j + 1; local_j++)
                {
                    ...
                    {
                        tempRed += image[local_i][local_j].rgbtRed;     // red
                        tempGreen += image[local_i][local_j].rgbtGreen; // green
                        tempBlue += image[local_i][local_j].rgbtBlue;   // blue

                        pixelCount++;
                    }
                }
            }
            // update the pixel value to have the average value
