...
# Configure the CS50 Library to use SQLite database
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("Symbol is required.Please check")
        elif not shares or not shares.isdigit() or int(shares) <= 0:
            return apology("Must be a positive number of shares.Please check")

        quote = lookup(symbol)
        if quote is None:
            return apology("Symbol not found.Please check")
        ...
        if cash < total_cost:
            return apology("You don't have enough cash.Please check")
...
def login():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            return apology("must provide username.Please check", 403)

        # Ensure the password was submitted
        elif not request.form.get("password"):
            return apology("must provide password.Please check", 403)
        ...
        if len(rows) != 1 or not check_password_hash(
            ...
            return apology("invalid username and/or password.Please check", 403)
...
def register():
    ...
    if request.method == "POST":
        # The credentials
        if not request.form.get("username"):
            return apology("Username is required.Please check", 400)
        elif not request.form.get("password"):
            return apology("Password is required.Please check", 400)
        elif not request.form.get("confirmation"):
            return apology("You must confirm your password.Please check", 400)
        elif request.form.get("password") != request.form.get("confirmation"):
            return apology("Passwords do not match.Please check", 400)
        ...
        if len(rows) != 0:
            return apology("Username already exists.Please check", 400)

        # register new valid user here
        db.execute("INSERT INTO users (username, hash) VALUES (?, ?)", request.form.get(
            "username"), generate_password_hash(request.form.get("password")))

        # search for the newly registered user here
        rows = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))

        # create a new session for the user here
        session["user_id"] = rows[0]["id"]
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    return apology("You don't have enough shares.Please check")
                else:
                    quote = lookup(symbol)
                    if quote is None:
                        return apology("Symbol not found.Please check")
                    ...
                    flash(f"Sold {shares} the shares of {symbol} for {usd(total_sale)}!")
                    return redirect("/")
